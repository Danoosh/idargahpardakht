...
class iDargahPardakht extends PaymentModule
{  
	private $_html = '';

	private  $_service_url = 'http://dargahpardakht.com/webservice/index.php';
	private $_go_url = 'http://dargahpardakht.com/webservice/go.php';
    private $_verify_url = 'http://dargahpardakht.com/webservice/verify.php';

	public function __construct(){  
		$this->name = 'idargahpardakht';
		...
		$this->displayName = $this->l('DargahPardakht Payment');  
		$this->description = $this->l('A free module to pay online.');  
		$this->confirmUninstall = $this->l('Are you sure, you want to delete your details?');
		if (!sizeof(Currency::checkPaymentCurrencies($this->id)))
			$this->warning = $this->l('No currency has been set for this module');
		$config = Configuration::getMultiple(array('IPRESTA_DARGAHPARDAKHT_UserName', 'IPRESTA_DARGAHPARDAKHT_UserPassword'));			
		if (!isset($config['IPRESTA_DARGAHPARDAKHT_UserName']))
			$this->warning = $this->l('Your DargahPardakht username must be configured in order to use this module');

	}  
	public function install(){
		if (!parent::install()
	    	OR !Configuration::updateValue('IPRESTA_DARGAHPARDAKHT_USER', '')
			OR !Configuration::updateValue('IPRESTA_DARGAHPARDAKHT_TEST', 0)
            OR !Configuration::updateValue('IPRESTA_DARGAHPARDAKHT_DEBUG', 0)
	...
	public function uninstall(){
		if (!Configuration::deleteByName('IPRESTA_DARGAHPARDAKHT_USER') 
            OR !Configuration::deleteByName('IPRESTA_DARGAHPARDAKHT_TEST')
			OR !Configuration::deleteByName('IPRESTA_DARGAHPARDAKHT_DEBUG')
			OR !parent::uninstall())
			return false;
		return true;
	}
	...
	public function renderForm()
	{
			...
			'form' => array(
				...
				'input' => array(
					array(
						...
						'name' => 'IPRESTA_DARGAHPARDAKHT_USER',
						'class' => 'fixed-width-lg',
					),
					array(
						'type' => 'switch',
						'label' => $this->l('Enable Debug Mode'),
						'name' => 'IPRESTA_DARGAHPARDAKHT_DEBUG',
		...
		$helper->submit_action = 'submitDargahPardakht';
	...
	{
		return array(
			'IPRESTA_DARGAHPARDAKHT_USER' => Tools::getValue('IPRESTA_DARGAHPARDAKHT_USER', Configuration::get('IPRESTA_DARGAHPARDAKHT_USER')),
			'IPRESTA_DARGAHPARDAKHT_DEBUG' => Tools::getValue('IPRESTA_DARGAHPARDAKHT_DEBUG', (bool)Configuration::get('IPRESTA_DARGAHPARDAKHT_DEBUG')),
		);
	}
    ...
    public function getContent()
	{
		$output = '';
		$errors = array();
		if (isset($_POST['submitDargahPardakht']))
		{
			if (empty($_POST['IPRESTA_DARGAHPARDAKHT_USER']))
				$errors[] = $this->l('Your merchant code is required.');

			if (!count($errors))
			{
				Configuration::updateValue('IPRESTA_DARGAHPARDAKHT_USER', $_POST['IPRESTA_DARGAHPARDAKHT_USER']);
				Configuration::updateValue('IPRESTA_DARGAHPARDAKHT_DEBUG', $_POST['IPRESTA_DARGAHPARDAKHT_DEBUG']);
	...
	public function prePayment()
	{
        ...
        $id = Configuration::get('IPRESTA_DARGAHPARDAKHT_USER');
        $callback = $this->context->link->getModuleLink('idargahpardakht', 'validation');
